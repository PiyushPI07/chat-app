...
const validRegistration = function(details){
  ...
  if(!validator.isAlphanumeric(details.username)){
    // return { error: "Invalid Username, letters and numbers only."}
  ...
  if(!(details.password.match(pattern))){
    // return { error: "Password must have 1 uppercase, 1 lowercase, 1 number, 1 special character, and between 8 and 16 characters."}
  ...
  if(!validator.isAlpha(details.name.split(" ").join(""))){
    // return { error: "Name is supposed to be alphabets"}
    return [4, "Name is supposed to be alphabets"];
  }
  console.log("All checks passed, valid user");
  // return {message: "No input-validity errors!"}
  return [0, "No errors"];
}
